...
def test_search_renders_answer_sections():
    ...
    assert subheaders(app) == ["Ответ", "Таблица фактов", "Источники и цитаты", "Пробелы", "Карта связей ответа"]
    assert len(app.dataframe) == 1
    assert any("Пайплайн" in item.value and "1 факт" in item.value for item in app.caption)
